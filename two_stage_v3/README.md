...
| `architecture_v3.py` | `NeighborhoodAdapter` — self-context + delta encoder + attention + δ̂/σ²_pred head + (b)+residual δ_raw projection + Bayesian combination |
| `losses_v3.py` | `L_LOO` (Kendall) + decoder-stability losses + `L_epic_recon` |
| `training_v3.py` | Full-batch train loop + CLI; one adapter per condition |
| `inference_v3.py` | Apply trained adapter; dump `z_treat`, `delta_final`, `delta_hat`, `sigma2_pred`, `coherence` |
...
python -m two_stage_v3.inference_v3 \
