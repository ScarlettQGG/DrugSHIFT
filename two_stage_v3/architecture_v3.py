"""NeighborhoodAdapter — Stage 2 v3 architecture.

Forward pass per protein i under condition c:

    # (a) Self context — biology identity of i
    ctx_i  = MLP_self([z_i, h_image_i, h_seq_i, h_apms_i,
                       cluster_emb(cluster_id_i), cond_emb(c)])

    # (b) Encode each neighbour's δ_raw + h_*   (LEAVE-ONE-OUT: i's own δ excluded)
    e_j    = MLP_delta([δ_raw_j, h_image_j, h_seq_j, h_apms_j])  for j ∈ N(i)

    # (c) Attention over neighbours, modulated by graph confidence weights
    α_ij   = softmax_j(q(ctx_i) · k(e_j) / √d)  ·  knn_w[i, j]
    agg_i  = Σ_j α_ij · v(e_j)

    # (d) Predict neighbour-only delta + heteroscedastic σ²
    δ̂_i, log σ²_pred_i = Head([ctx_i, agg_i])

    # (e) δ_raw projection (b)+residual: frozen E_EPIC baseline + learned residual
    δ_raw_proj_i = E_EPIC(EPIC_treat_i) − E_EPIC(EPIC_ctrl_i)  +  ResidualMLP(δ_raw_i)
                                   └────────── frozen ──────────┘   └ zero-init ┘

    # (f) Bayesian combination with Stage-1's σ²_EPIC as σ²_raw
    σ²_raw_i = sigma2_epic_scale · σ²_EPIC_i (from cache)
    δ_final_i  = (σ²_pred · δ_raw_proj + σ²_raw · δ̂) / (σ²_raw + σ²_pred)

    z_i^treat = z_i + δ_final_i
"""
